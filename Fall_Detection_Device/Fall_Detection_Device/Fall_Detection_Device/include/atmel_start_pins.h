 ...
 * \param[in] pull_mode Pin pull mode
 */
static inline void USER_SW0_set_pull_mode(const enum port_pull_mode pull_mode)
{
	PORTC_set_pin_pull_mode(6, pull_mode);
}

/**
 * \brief Set USER_SW0 data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void USER_SW0_set_dir(const enum port_dir dir)
{
	PORTC_set_pin_dir(6, dir);
}

/**
 * \brief Set USER_SW0 level
 *
 * Sets output level on a pin
 *
 * \param[in] level true  = Pin level set to "high" state
 *                  false = Pin level set to "low" state
 */
static inline void USER_SW0_set_level(const bool level)
{
	PORTC_set_pin_level(6, level);
}

/**
 * \brief Toggle output level on USER_SW0
 *
 * Toggle the pin level
 */
static inline void USER_SW0_toggle_level()
{
	PORTC_toggle_pin_level(6);
}

/**
 * \brief Get level on USER_SW0
 *
 * Reads the level on a pin
 */
static inline bool USER_SW0_get_level()
{
	return PORTC_get_pin_level(6);
}

/**
 * \brief Set USER_LED0 pull mode
 *
 * Configure pin to pull up, down or disable pull mode, supported pull
 * modes are defined by device used
 *
 * \param[in] pull_mode Pin pull mode
 */
static inline void USER_LED0_set_pull_mode(const enum port_pull_mode pull_mode)
{
	PORTC_set_pin_pull_mode(7, pull_mode);
}

/**
 * \brief Set USER_LED0 data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void USER_LED0_set_dir(const enum port_dir dir)
{
	PORTC_set_pin_dir(7, dir);
}

/**
 * \brief Set USER_LED0 level
 *
 * Sets output level on a pin
 *
 * \param[in] level true  = Pin level set to "high" state
 *                  false = Pin level set to "low" state
 */
static inline void USER_LED0_set_level(const bool level)
{
	PORTC_set_pin_level(7, level);
}

/**
 * \brief Toggle output level on USER_LED0
 *
 * Toggle the pin level
 */
static inline void USER_LED0_toggle_level()
{
	PORTC_toggle_pin_level(7);
}

/**
 * \brief Get level on USER_LED0
 *
 * Reads the level on a pin
 */
static inline bool USER_LED0_get_level()
{
